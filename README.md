...
- **Single binary, three things.** One Rust binary acts as installer (first
  ...
  engine with GitOps mode). See spec §2.1.
...
- **Native install everywhere.** Linux (systemd), macOS (launchd), Windows
  ...
  v1.0. Kubernetes and cloud drivers are deferred to v1.2. See spec §10.
...
- **The control plane manages the lakehouse; the control plane is not the
  lakehouse.** If Computeza stops, the data plane keeps serving. See spec §3.1.

## Workspace layout

The crates follow the four tiers from spec §3.2.
...
Provider channel program — see spec §11. The repository is not licensed for
redistribution.
