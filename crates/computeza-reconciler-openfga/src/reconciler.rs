...
impl<D: Driver> OpenFgaReconciler<D> {
    ...
    /// Persist the latest status under `openfga-instance/<instance_name>`
    /// if a state store is attached. Best-effort.
    async fn persist_status(&self, status: &OpenFgaStatus) {
        let Some(binding) = &self.state else { return };
        let key = ResourceKey::cluster_scoped("openfga-instance", &binding.instance_name);
        let status_json = match serde_json::to_value(status) {
            Ok(v) => v,
            Err(e) => {
                warn!(error = %e, "failed to serialize OpenFgaStatus for state persistence");
                return;
            }
        };
    ...
    async fn observe(&self, _ctx: &Context) -> Result<OpenFgaStatus, CoreError> {
        let status = match self.snapshot().await {
            ...
            Err(e) => {
                warn!(
                    error = %e,
                    "openfga observe failed; returning sentinel status with last_observe_failed=true. \
                     Reconciler will retry on next tick. Check: (1) HTTP port reachable, \
                     (2) admin token / preshared key valid, (3) OpenFGA has finished startup."
                );
                OpenFgaStatus {
