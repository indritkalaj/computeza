...
ui-workspace-catalog-empty      = No namespaces yet. Use the form below to create your first one (dotted names like `finance.raw` become nested namespaces). The SQL editor on the right talks to Databend, which is a separate system from the Iceberg catalog -- `CREATE NAMESPACE` from SQL won't help here.
ui-workspace-catalog-create-label = New namespace
ui-workspace-catalog-create-placeholder = finance.raw
ui-workspace-catalog-create-button = Create
ui-workspace-catalog-create-help = Creates an Iceberg namespace in Lakekeeper. v0.0.x scope: namespace-only -- create tables via the SQL editor (Databend) or an external Iceberg-aware tool (pyiceberg, Spark, Trino).
