...
ui-studio-lakekeeper-warehouses-note = Click a warehouse name to browse namespaces + tables inside it. Row-level preview (querying table contents from the Studio) lands in phase 1.6 once Databend's `[[catalog]]` block is wired to Lakekeeper.
ui-studio-history-heading = Recent queries
ui-studio-history-empty   = (no queries yet)
ui-studio-history-reload  = Reload
ui-studio-bootstrap-title = Lakekeeper bootstrap (recovery path)
ui-studio-bootstrap-intro = Auto-bootstrap is the primary path -- a fresh `Install Garage` + `Install Lakekeeper` from /install creates the project + warehouse for you and surfaces credentials in the credentials.json download. This page is the power-user escape hatch: use it when auto-bootstrap fails (the install-result page will say so) or when migrating from v0.0.x where you already minted Garage credentials by hand. Vault-stored credentials are pre-filled below; submit to (re-)provision the warehouse.
ui-studio-bootstrap-link  = Open the manual bootstrap form (recovery) >
